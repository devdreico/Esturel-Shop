        ...
        <>
          <ul className="mt-8 space-y-3">
            {items.map(({ product, qty, lineTotal }) => (
              <li key={product.id} className="glass-card flex flex-wrap items-center gap-4 rounded-2xl p-4">
                <img
                  src={product.image}
                  alt=""
                  className="h-16 w-16 rounded-xl bg-surface object-cover"
                />
                <div className="min-w-0 flex-1">
                  <Link to={`/producto/${product.slug}`} className="font-bold hover:text-urple-500">
                    {product.name}
                  </Link>
                  <p className="text-xs text-muted">{formatCOP(product.priceCOP)} c/u</p>
                </div>
                ...
                <p className="w-28 text-right text-sm font-bold">{formatCOP(lineTotal)}</p>
                <button
                  type="button"
